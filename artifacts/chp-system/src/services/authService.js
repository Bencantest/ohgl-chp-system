...
function authErrorMessage(err, fallback) {
  ...
  // Email rate-limit hit by Supabase free tier (≤3 emails/hour)
  if (
    /over.*email.*rate.*limit|email.*rate.*limit|rate.*limit.*email|for.*security.*purposes.*only.*request.*email/i.test(message) ||
    err?.status === 429 ||
    err?.code === 'over_email_rate_limit' ||
    err?.code === 'email_rate_limit_exceeded' ||
    /too many/i.test(message)
  ) {
    return (
      'Registration emails are temporarily paused — the system has reached its hourly email limit. ' +
      'Please wait at least 1 hour and try again, or ask an admin to create your account via the Invite Staff flow.'
    );
  }
