...
import { updateReferralSecureFull, deleteReferralRecord, writeAuditLog } from '../services/dataService.js';
...
function updateSaveButtonsState() {
  ...
  // clinician and receptionist both get the manual save workflow
  const isManualSaveRole = role === 'facility_manager' || role === 'facility_officer'
    || role === 'clinician' || role === 'receptionist';
...
export function renderTracker(nextPage = trackerPage) {
  ...
  let refs = [...(f.referrals || [])].reverse().filter(r => {
    ...
    const haystack = [
      r.patient, r.id, r.chp_code, chp.name, chp.phone, chp.village, chp.unit,
      r.chp_unit, r.category, r.priority, r.sha, r.complaint, r.notes,
      r.received_by, r.file_no, r.sha_no, r.date,
...
export async function executeReferralWorkflowAction(referralId, command) {
  ...
  try {
    ...
    // Audit log for Received / In Review transitions — required for clinician & receptionist workflow.
    // Use the referral's actual workflow_status from the refreshed DB (authoritative), with the action
    // label as a secondary check for when the referral isn't found in local state.
    const AUDIT_STATUSES = ['Received', 'In Review'];
    let actualNewStatus = '';
    for (const f of DB.facilities) {
      const found = f.referrals?.find(r => r.db_id === referralId || r.id === referralId);
      if (found) { actualNewStatus = found.workflow_status || found.status || ''; break; }
    }
    const isAuditableTransition = AUDIT_STATUSES.some(s =>
      s.toLowerCase() === actualNewStatus.toLowerCase()
    ) || /received|in.?review/i.test(action.label);
    if (isAuditableTransition && currentUser) {
      try {
        await writeAuditLog({
          ...
          changes: {
            ...
            new_status: actualNewStatus || action.label,
            actor_role: currentProfile?.role,
            timestamp: new Date().toISOString(),
          },
        });
      } catch (auditErr) {
        console.warn('Workflow audit log failed', auditErr);
      }
    }
...
export async function updRef(facId, i, field, val) {
  ...
  if (dbField && r.db_id) {
    ...
    if (isManualSaveRole) {
      if (!trackerEdits[r.db_id]) trackerEdits[r.db_id] = {};
...
window.saveTrackerChanges = async function() {
  ...
  let successCount = 0;
  const errors = [];
  for (const refDbId of Object.keys(trackerEdits)) {
  ...
  if (errors.length > 0) {
    showTrackerAlert(`Saved ${successCount} successfully. Failed: ${errors.length}:\n` + errors.join('\n'), 'alert-e');
    updateSaveButtonsState();
  } else {
    trackerEdits = {};
